...
const db = mysql.createPool({
  host: "51.38.205.167",
  user: "u19638_tkObFcb2Lo",
  password: "+5A!=Q^.e.9NUPDk1ZdLNadH",
  database: "s19638_newlife",
...
app.get("/jobs/top/:jobid", async (req, res) => {
  ...
  try {
    const [rows] = await db.query(`
      SELECT playerid AS username, SUM(amount) AS total
      FROM job_logs
      WHERE jobid = ?
      GROUP BY playerid
      ORDER BY total DESC
...
app.get("/contracts/logs", async (req, res) => {
  const page = Number(req.query.page) || 1;
  const limit = 50;
  const offset = (page - 1) * limit;
  ...
  try {
    const [rows] = await db.query(
      "SELECT id, date, description FROM log_contracts ORDER BY id DESC LIMIT ? OFFSET ?",
      [limit, offset]
    );
...
app.get("/vip/logs", async (req, res) => {
  const page = Number(req.query.page) || 1;
  const limit = 50;
  const offset = (page - 1) * limit;
  ...
  try {
    const [rows] = await db.query(
      "SELECT id, date, description FROM log_vip ORDER BY id DESC LIMIT ? OFFSET ?",
      [limit, offset]
    );
    ...
    console.error(err);
    res.status(500).json({ error: "Failed to load VIP logs" });
  }
});
/* FACTION LOGS */
